...
### 4.8 Go/Fyne/CLI Implementation References
...
|---|---|---|
| Desktop UI | Fyne `fyne-io/fyne` | Cross-platform Go desktop application. |
...
- Track repositories by topic: scholarly PDF parsing, metadata APIs, systematic review, meta-analysis, search, vector databases, scientific NLP, citation graphs, Fyne UI, Go CLI tooling.
...
```text
 ...
 ┌──────────────────▼──────────────────┐
 │ Go application core                  │
 │ shared domain services for CLI/Fyne  │
 └──────────────────┬──────────────────┘
                    │
 ┌──────────────────▼──────────────────┐
 │ Ingestion layer                      │
...
- Application core: Go domain services shared by CLI and Fyne UI
...
- UI prototype: Fyne from the start, with CLI parity for automation
...
- Fyne examples and demo apps — idiomatic Go desktop layout, widgets, dialogs, data binding.
- Zotero desktop UX — library/sidebar/detail-pane/notes workflow.
...
Core workflows must be available from both CLI and Fyne UI where practical. The CLI is required for automation and reproducibility; the Fyne UI is required for interactive research, screening, extraction, graph exploration, and visual analysis.

### 8.6 Go-First Maintainability

The core product should remain Go-first. External systems such as GROBID, R `metafor`, Python NLP tools, or vector databases may be used through stable adapters, subprocesses, containers, or HTTP APIs, but the project orchestration, data model, CLI, and Fyne UI should be owned in Go.
...
### Phase 0: Go/Fyne/CLI Foundation

- Create Go module and application architecture.
- Define shared domain services used by both CLI and Fyne UI.
- Implement CLI skeleton.
- Implement Fyne desktop shell.
...
- Build first Fyne screens for project dashboard, search, and library.
...
- CLI and Fyne export flows

---

## 10. Recommended Study Order

1. Go/Fyne architecture — desktop UI patterns, background jobs, thread-safe UI updates.
...
| Fyne UI blocking/background work | Keep long-running parsing/indexing/network jobs off the UI path and update widgets safely from the UI thread. |
...
1. Create a research project from the CLI and the Fyne UI.
...
9. Study and catalog relevant open-source repositories from the CLI and view them in Fyne.
10. Export a reproducible report with citations and provenance.
