...
- [x] Add SciSpaCy-inspired scientific entity extraction suggestions with passage offsets, abbreviation resolution, entity-link candidates, confidence, model provenance, and reviewer decisions (`EveryScientificEntitySuggestionAuditable`, `rforge evidence entity-suggest|entity-review`, `opensource/inventory/scispacy.md`).
- [x] Add KeyBERT-inspired keyword/query-expansion suggestions linked to abstracts/passages, with diversity scoring, reviewer approval, and before/after search-plan provenance (`DraftQueryExpansionSuggestions`, `ApplyApprovedQueryExpansions`, `opensource/inventory/keybert.md`).
- [x] Add NASA ADS connector planning and implementation slices for bibcode/DOI search, physics/astronomy metadata normalization, citation expansion, token redaction, and opt-in live smoke tests (`NASAADSConnector`, `NewNASAADSHTTPClient`, `RFORGE_ADS_TOKEN`, `opensource/inventory/nasa-ads.md`).
- [x] Add DOAJ/CORE open-access discovery slices for license-aware full-text candidate queues, source URL provenance, attribution/rate-limit handling, and reviewer-approved acquisition (`CompareOpenAccessCandidates`, `BuildLegalAcquisitionQueue`, `/acquisition`, `opensource/inventory/doaj-core.md`).
