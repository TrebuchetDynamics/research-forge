...
- [ ] Add Zotero RDF import/export with collection hierarchy, Better BibTeX citation-key preservation, annotation mapping, and privacy-redacted attachment metadata (`opensource/inventory/zotero.md`).
- [ ] Persist ASReview-style active-learning prioritization runs with dataset/input hashes, seed decisions, ranking method, ranked output, reviewer progress metrics, stopping diagnostics, and adjudication state (`opensource/inventory/asreview.md`).
- [ ] Add a parser quality/confidence report that compares GROBID, S2ORC-style JSON, PaperMage, CERMINE, Science Parse-style metadata, and Anystyle reference output without auto-accepting conflicting fields (`opensource/inventory/{grobid,s2orc-doc2json,papermage,cermine,science-parse,anystyle}.md`).
- [ ] Normalize parsed bibliography references against Crossref, OpenAlex, and Semantic Scholar while preserving raw reference text, parser provenance, match confidence, and reviewer-visible ambiguity queues (`opensource/inventory/{grobid,s2orc-doc2json,anystyle}.md`).
- [ ] Import full bibliography-to-citation-graph edges from parsed documents and link citation-span offsets back to passages and report evidence (`opensource/inventory/{grobid,s2orc-doc2json}.md`).
- [ ] Add per-passage provenance and parser/version/source-offset links in generated reports so evidence claims remain traceable after parser upgrades (`opensource/inventory/{grobid,papermage}.md`).
- [ ] Extend the PaperMage/S2ORC-style parsed-document model with layered annotations, stable offsets, citation spans, and multi-parser reconciliation outputs (`opensource/inventory/{papermage,s2orc-doc2json}.md`).
- [ ] Add a CERMINE adapter seam and fallback orchestration policy, including a review UI for conflicting parser outputs (`opensource/inventory/cermine.md`).
- [ ] Add stale-parser maintenance/risk scoring and comparative benchmark fixtures for parser candidates before enabling historical fallbacks such as Science Parse (`opensource/inventory/science-parse.md`).
- [ ] Add OpenSearch mapping-version lockfile records, bulk indexing with partial-failure provenance, highlighted passage search results, and an opt-in OpenSearch integration test (`opensource/inventory/opensearch.md`).
- [ ] Add a Qdrant adapter implementation with production embedding-provider presets, payload privacy/version locking, calibrated hybrid ranking controls, and an opt-in Qdrant integration test (`opensource/inventory/qdrant.md`).
- [ ] Add richer OpenAlex cursor-based multi-page import runs with frozen query/cursor state, concept/domain-map import, higher-level filter presets, and opt-in live smoke coverage for those workflows (`opensource/inventory/openalex.md`).
- [ ] Add Semantic Scholar quota-aware resumable graph expansion run files plus richer live smoke coverage for API drift and field restrictions (`opensource/inventory/semantic-scholar.md`).
- [ ] Add richer interactive graph exploration beyond the artifact preview: filtering, neighborhood expansion, provenance overlays, and keyboard-accessible alternatives (`opensource/inventory/semantic-scholar.md`).
- [ ] Add additional metafor-backed effect-size calculators, influence diagnostics, richer publication-bias tests, publication-ready plot styling, and a separate Bayesian-engine path beyond the current normal-approximation scaffold (`opensource/inventory/metafor.md`).
- [ ] Add a reference-manager interchange fidelity matrix that round-trips BibTeX, RIS, CSL-JSON, Zotero RDF, Better BibTeX citation keys, tags, notes, collections, and redacted attachment metadata with per-field loss reports (`opensource/inventory/zotero.md`).
- [ ] Add a privacy/licensing review gate for imported reference-manager attachments, notes, and annotations before they can be copied into project archives or shareable reports (`opensource/inventory/zotero.md`).
- [ ] Add an ASReview-style audit export bundle containing frozen screening dataset, seed labels, ranking iterations, reviewer actions, stopping diagnostics, random seeds, and model/version metadata (`opensource/inventory/asreview.md`).
- [ ] Add biomedical full-text workflows for PubMed/Europe PMC/PMC open-access records: PMCID/PMID linking, OA license capture, structured full-text import, supplementary-file discovery, and opt-in live drift smoke tests (`opensource/inventory/README.md`, `docs/source-connectors.md`).
- [ ] Add a source-API drift dashboard that records live-smoke snapshots for OpenAlex, Semantic Scholar, PubMed, Europe PMC, Crossref, arXiv, and Unpaywall, flags response-shape changes, and links failures to connector provenance (`opensource/inventory/README.md`).
- [ ] Add retrieval evaluation benchmarks comparing SQLite FTS, OpenSearch, Qdrant vector search, and hybrid ranking on deterministic passage-query fixtures with reproducibility and privacy notes (`opensource/inventory/{opensearch,qdrant}.md`).
- [ ] Add an OSS-inventory-to-roadmap report that groups `nextSlice` entries by area, detects TODO coverage gaps for new inventory notes, and suggests adapter/test/live-smoke implementation slices without marking them complete (`opensource/inventory/manifest.json`).
- [ ] Add reviewer-persistent parsed-reference adjudication: reviewers can accept, correct, reject, or defer Anystyle/GROBID/S2ORC reference matches, with decision provenance and exportable ambiguity queues (`opensource/inventory/anystyle.md`).
- [ ] Add embedding-provider compliance profiles that document what text leaves the machine, required consent/config, model version locks, dimensionality, retention policy, and redaction behavior before Qdrant/HTTP embedding indexing runs (`opensource/inventory/qdrant.md`).
- [ ] Add calibrated hybrid retrieval tuning files that store lexical/vector/backend weights, evaluation scores, selected configuration, and query-set checksums for reproducible retrieval comparisons (`opensource/inventory/{opensearch,qdrant}.md`).
- [ ] Add OpenAlex author/institution/concept disambiguation review queues so imported domain maps and entity searches do not silently merge ambiguous people, institutions, or concepts (`opensource/inventory/openalex.md`).
- [ ] Add Semantic Scholar/OpenAlex graph expansion budget controls: max depth, max nodes, max API calls, retry budget, resume cursor, and dry-run estimate before live citation expansion (`opensource/inventory/{semantic-scholar,openalex}.md`).
- [ ] Add publication-ready analysis artifact manifests that bundle forest/funnel SVGs, plot settings, checksums, R/metafor script, engine versions, warnings, and report embedding metadata (`opensource/inventory/metafor.md`).
- [ ] Add parser-output license/provenance manifests for external parsers so generated TEI/JSON/reference files record parser source, version, command, input checksum, output checksum, license constraints, and shareability (`opensource/inventory/{grobid,s2orc-doc2json,cermine,anystyle}.md`).
- [ ] Add accessibility and no-JS review views for large citation/domain graphs: tabular edge lists, filtered node tables, keyboard navigation, graph summaries, and exportable graph reports alongside interactive SVGs (`opensource/inventory/{semantic-scholar,openalex}.md`).
...
- [ ] Add a live research map cockpit that combines concept maps, citation neighborhoods, screening priority, parser quality, retrieval hits, and evidence coverage into one local web dashboard with no-JS fallbacks and exportable audit snapshots (`opensource/inventory/{openalex,semantic-scholar,asreview,qdrant,opensearch}.md`).
- [ ] Add an evidence gap analyzer that cross-checks the research question, screened-in papers, parsed passages, extracted evidence fields, and meta-analysis inputs to identify missing outcomes, missing comparators, unsupported claims, and studies needing full-text acquisition (`opensource/inventory/{asreview,grobid,papermage,metafor}.md`).
- [ ] Add a reproducible-review package format that bundles project manifest, lockfiles, source query plans, dedupe decisions, parser manifests, screening audit, extraction schema, analysis artifacts, report outputs, redaction policy, and checksums for archival or peer review (`opensource/inventory/{zotero,asreview,grobid,metafor}.md`).
- [ ] Add a connector capability registry that describes each source/tool adapter's supported entities, rate limits, auth needs, live-smoke status, license/shareability policy, cacheability, and supported provenance fields (`opensource/inventory/manifest.json`).
- [ ] Add a cross-tool benchmark suite that measures discovery recall, dedupe precision, parser field accuracy, reference-normalization accuracy, retrieval quality, screening effort savings, and report reproducibility on deterministic fixture projects (`opensource/inventory/{openalex,semantic-scholar,grobid,anystyle,asreview,opensearch,qdrant}.md`).
- [ ] Add an LLM-assisted but citation-locked synthesis layer that can suggest query expansions, screening rationales, extraction candidates, and report prose only when every suggested sentence is linked to exact source passages/evidence and marked unaccepted until reviewer approval (`RESEARCH-FORGE-PRD.md`, `opensource/inventory/{grobid,papermage,asreview}.md`).
- [ ] Add a lab-notebook timeline that records all human and automated workflow events across imports, source refreshes, parser runs, reviewer decisions, extraction edits, analysis reruns, and report builds as a browsable provenance journal (`opensource/inventory/{zotero,asreview,metafor}.md`).
- [ ] Add a method-comparison workbench where users can compare parser choices, retrieval backends, screening rankers, effect-size models, and publication-bias diagnostics side-by-side before selecting the method locked into the final report (`opensource/inventory/{grobid,cermine,opensearch,qdrant,asreview,metafor}.md`).
- [ ] Add a one-command `rforge forge` guided workflow that creates or opens a project, asks for the research question and source/tool choices, previews privacy/legal implications, then drives the unified orchestrator with review gates at every irreversible scientific or data-sharing decision (`RESEARCH-FORGE-PRD.md`, `opensource/inventory/README.md`).
...
- [ ] Write a ResearchForge super-tool blueprint that maps each open-source-inspired capability to a first-class ResearchForge module, CLI command family, HTMX page, storage tables/files, provenance events, and validation gate, prioritizing the systematic-review/meta-analysis path before broad general-purpose research-cockpit expansion (`RESEARCH-FORGE-PRD.md`, `opensource/inventory/README.md`, `docs/web-gui-plan.md`).
- [ ] Add a phased super-tool roadmap that makes systematic review/meta-analysis the first product spine: source plan → import/dedupe → legal full text → parser arbitration → screening → evidence extraction → statistics → auditable report, then layers broader knowledge-cockpit features after that spine is reproducible (`RESEARCH-FORGE-PRD.md`, `TODO.md`).
...
- [ ] Add HTMX dashboard requirement: a legal full-text acquisition queue showing OA/license status, source URL, expected stored path, restricted/shareable flags, and explicit reviewer approval before downloading or archiving documents (`RESEARCH-FORGE-PRD.md`).
- [ ] Add HTMX dashboard requirement: a parser arbitration screen comparing GROBID, S2ORC-style JSON, PaperMage, CERMINE, Science Parse-style metadata, and Anystyle outputs field-by-field with confidence, raw text, offsets, warnings, and accept/correct/reject controls (`opensource/inventory/{grobid,s2orc-doc2json,papermage,cermine,science-parse,anystyle}.md`).
- [ ] Add HTMX dashboard requirement: a retrieval tuning screen comparing SQLite FTS, OpenSearch, Qdrant vector, and hybrid results for the same query, with passage provenance, ranking explanations, embedding privacy status, and benchmark scores (`opensource/inventory/{opensearch,qdrant}.md`).
- [ ] Add HTMX dashboard requirement: an ASReview-inspired screening cockpit with active-learning queue, uncertainty/exploration flags, reviewer assignment, conflict/adjudication panels, recall/effort curves, stopping diagnostics, and exportable audit bundle links (`opensource/inventory/asreview.md`).
- [ ] Add HTMX dashboard requirement: an evidence extraction grid linking each field to source passage/table/figure/equation, parser offset, PDF view, reviewer status, correction history, and downstream analysis inclusion status (`opensource/inventory/{grobid,papermage,metafor}.md`).
- [ ] Add HTMX dashboard requirement: a meta-analysis workbench showing prepared effect-size inputs, model choices, metafor script, warnings, heterogeneity, sensitivity/influence diagnostics, forest/funnel artifacts, and publication-ready artifact manifests (`opensource/inventory/metafor.md`).
- [ ] Add HTMX dashboard requirement: a claim traceability panel in the report builder where every generated paragraph/table/figure references accepted evidence and unresolved or weakly supported claims block final export (`RESEARCH-FORGE-PRD.md`).
- [ ] Add HTMX dashboard requirement: a research map view that unifies citation graph, OpenAlex concepts, Zotero collections/tags, screening status, retrieval clusters, and evidence coverage, with filters, keyboard navigation, no-JS tables, and snapshot export (`opensource/inventory/{openalex,semantic-scholar,zotero,asreview}.md`).
- [ ] Add HTMX dashboard requirement: a connector health/control center showing live-smoke history, API drift alerts, cache status, credential redaction checks, rate-limit budgets, and adapter capability registry coverage (`opensource/inventory/manifest.json`).
- [ ] Add HTMX dashboard requirement: a reproducibility/export center that previews archive contents, redaction results, checksums, lockfiles, external-tool versions, parser manifests, analysis artifacts, and reviewer decision logs before package creation (`opensource/inventory/{zotero,grobid,asreview,metafor}.md`).
