...
- `rforge index rebuild [--backend sqlite|opensearch|qdrant|hybrid]` (writes `data/retrieval.lock.json`; OpenSearch also writes `data/opensearch.mapping.lock.json` and `data/opensearch.bulk-report.json`; qdrant/hybrid honor `RFORGE_EMBEDDING_DIMENSIONS` or opt-in `RFORGE_EMBEDDING_URL`/`RFORGE_EMBEDDING_MODEL`), `rforge retrieve --query <query> [--backend sqlite|opensearch|qdrant|hybrid]` (OpenSearch results include highlighted passage snippets when returned by the backend)
