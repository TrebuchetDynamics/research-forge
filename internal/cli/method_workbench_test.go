...
func TestExecuteAnalysisMethodWorkbenchWritesComparison(t *testing.T) {
	...
	code := Execute([]string{"--json", "--project", project, "analysis", "method-workbench", "run1", "--category", "effect-size models", "--method", "smd", "--method", "risk-difference", "--out", out}, &stdout, &stderr)
	if code != 0 {
	...
	if report.Category != "effect-size models" || len(report.Options) != 2 || !report.RequiresReviewerChoice {
