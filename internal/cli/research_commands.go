...
func executeResearchParsePDFText(args []string, stdout, stderr io.Writer, opts globalOptions) int {
	...
	text, err := exec.CommandContext(context.Background(), "pdftotext", "-layout", values["--pdf"], "-").Output()
	if err != nil {
	...
	if opts.JSON {
		return writeJSON(stdout, 0, map[string]any{"path": values["--out"], "paper": values["--paper"], "parser": doc.ParserName, "passages": len(doc.Sections[0].Passages)})
	}
