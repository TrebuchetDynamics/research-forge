...
func TestBuildLegalAcquisitionQueueCapturesPolicyFields(t *testing.T) {
	comparison := sources.OpenAccessCandidateComparison{SchemaVersion: "1", Candidates: []sources.OpenAccessCandidate{
		{PaperTitle: "OA paper", DOI: "10.1000/oa", Source: "unpaywall", URL: "https://example.org/paper.pdf", License: "CC-BY", OAStatus: "gold", Provenance: "unpaywall:/v2/10.1000/oa", ReviewerApprovalRequired: true},
	...
	open := queue.Items[0]
	if open.OAStatus != "gold" || open.License != "CC-BY" || open.SourceURL != "https://example.org/paper.pdf" || open.ExpectedLocalPath != filepath.Join("/project", "documents", "open-access", "10-1000-oa.pdf") {
