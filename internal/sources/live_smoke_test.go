...
func liveSmokeHTTPClient(baseURLEnv, defaultURL string) HTTPClient {
	return liveSmokeHTTPClientWithHeaders(baseURLEnv, defaultURL, nil)
}

func liveSmokeHTTPClientWithHeaders(baseURLEnv, defaultURL string, headers map[string]string) HTTPClient {
	baseURL := os.Getenv(baseURLEnv)
	if baseURL == "" {
		baseURL = defaultURL
	}
	return NewHTTPClient(HTTPClientOptions{BaseURL: baseURL, UserAgent: "ResearchForge-smoke/dev", Timeout: 30 * time.Second, MaxRetries: 2, Headers: headers})
}

func semanticScholarLiveSmokeHTTPClient() HTTPClient {
	headers := map[string]string{}
	if apiKey := strings.TrimSpace(os.Getenv("RFORGE_SEMANTIC_SCHOLAR_API_KEY")); apiKey != "" {
		headers["x-api-key"] = apiKey
	}
	return liveSmokeHTTPClientWithHeaders("RFORGE_SEMANTIC_SCHOLAR_URL", "https://api.semanticscholar.org", headers)
}
...
func TestOptInLiveSourceConnectorSmoke(t *testing.T) {
	...
	searchConnectors := []struct {
		...
		{"semantic-scholar", "RFORGE_SEMANTIC_SCHOLAR_URL", "https://api.semanticscholar.org", func(HTTPClient) SourceConnector {
